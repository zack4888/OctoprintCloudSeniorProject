...
#from octoprint.printer.standard import *
...
class MqttPlugin(octoprint.plugin.SettingsPlugin,
    ...
    def get_assets(self):
        return dict(js=["js/OctoprintCloudSeniorProject.js"])

    ##~~ StartupPlugin API

    def on_startup(self, host, port):
        self.mqtt_connect()
        if self._settings.get_boolean(["subscription","active"]) == True:
            topic = self._settings.get(["publish", "baseTopic"]) + self._settings.get(["subscription", "topic"])
            self._logger.info(topic)
            self.mqtt_subscribe(topic,self.message_received)
            self._logger.info("subscription active")
    ...
    def on_settings_save(self, data):
        ...
        if self._settings.get_boolean(["subscription","active"]) == True:
            ...
            self._logger.info("subscription active")
        elif self._settings.get_boolean(["subscription","active"]) == False:
            self._logger.info("subscription deactivated")
    
    def on_message(self, userdata, message):
        self._logger.info("received message =",str(message.payload.decode("utf-8")))
    ...
    def _on_mqtt_message(self, client, userdata, msg):
        ...
        self._logger.info("received message = "+str(msg.payload.decode("utf-8")))
        ...
        for subscription in self._mqtt_subscriptions:
            ...
            if topic_matches_sub(topic, msg.topic):
                args = [msg.topic, msg.payload.decode("utf-8")] + args
    ...
    def message_received(self, something, topic, payload, *args, retained=False, qos=0, **kwargs):
        import octoprint.printer 
        self._logger.info("payload = "+str(payload))
        self._logger.info("topic = "+str(topic))
        self._logger.info("retained? = "+str(retained))
        self._logger.info("qos? = "+str(qos))
